...
function onCardYearChange(year) {
  ...
  // --- Period card calculations ---
  const periodData = useMemo(() => {
    const lastWeekStart = getLastWeekStart();

    if (period === "week") {
      return {
        label:        `Last week (${lastWeekStart})`,
        oee:          summary.oee,
        ...
        dpuColor: (() => {
          ...
          const prev = allHistory[allHistory.length - 2]?.dpu ?? null;
          if (!prev) return "#888";
          return last < prev ? "#1D9E75" : last > prev ? "#A32D2D" : "#888";
        })(),
    ...
    const avgPerformance = weeklyTarget > 0 ? Math.round(avgTrucks / weeklyTarget * 100 * 10) / 10 : 0;
    ...
    const avgAvail = periodIndirect.length > 0
      ? Math.round(periodIndirect.reduce((s, r) => {
          const avail = r.total_labor_hours > 0
            ? ((r.total_labor_hours - r.indirect_hours - r.rework_hours) / r.total_labor_hours) * 100
            : 0;
          return s + avail;
        }, 0) / periodIndirect.length * 10) / 10
      : 0;

    const avgQuality = avgDpu === 0 ? 0
      : avgDpu <= goals.quarterly_dpu_goal ? 100
      : Math.round(goals.quarterly_dpu_goal / avgDpu * 100 * 10) / 10;

    const avgOee = Math.round(avgAvail / 100 * avgPerformance / 100 * avgQuality / 100 * 100 * 10) / 10;
  ...
  ytd:     `Full year ${cardYear}`,
    ...
    return {
      ...
      dpu:          avgDpu,
      dpuArrow:     "",
      dpuColor:     avgDpu <= goals.quarterly_dpu_goal ? "#1D9E75" : "#A32D2D",
      trucksLabel:  "Avg trucks / week",
      dpuLabel:     "Avg DPU",
      trucksSub:    `Target: ${goals.weekly_trucks_min}–${goals.weekly_trucks_max}`,
      dpuSub:       `${periodLabels[period]} average`,
    };
  }, [period, summary, allHistory, indirectHistory, goals, currentYear, currentMonth, currentQuarter, cardYear, cardSub]);
  ...
  const qdpuColor      = periodData.dpu === 0 ? "#aaa" : periodData.dpu <= goals.quarterly_dpu_goal ? "#1D9E75" : "#A32D2D";
...
<p style={{ fontSize: 11, color: "#aaa", margin: "0 0 14px" }}>{periodData.label}</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 10 }}>
        ...
        <MetricCard label="Quality"      value={`${periodData.quality}%`}      sub="Quarterly goal ÷ actual DPU"                    color={qualColor} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 24 }}>
        ...
        <div style={{ background: "#fafafa", borderRadius: 8, padding: "14px 16px", border: "0.5px solid #eee" }}>
          ...
          <p style={{ fontSize: 22, fontWeight: 500, margin: "0 0 4px", color: periodData.dpu === 0 ? "#aaa" : periodData.dpu <= goals.quarterly_dpu_goal ? "#1D9E75" : "#A32D2D" }}>
        ...
        <div style={{ background: "#fafafa", borderRadius: 8, padding: "14px 16px", border: "0.5px solid #eee" }}>
          ...
          <p style={{ fontSize: 22, fontWeight: 500, margin: "0 0 4px", color: summary.yearly_dpu === 0 ? "#aaa" : summary.yearly_dpu <= goals.annual_dpu_goal ? "#1D9E75" : "#A32D2D" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {["month", "quarter", "ytd"].map(f => (
                <button key={f} onClick={() => onDpuFilterChange(f)} style={btnStyle(dpuFilter === f)}>
